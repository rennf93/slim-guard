# slim-guard advanced app

Production-shaped guarded Slim 4 app: env-driven `SecurityConfig` engine
...
- `public/index.php`: app assembly (engine, guard, routes, run)
- `src/Config.php`: env-driven `SecurityConfig` plus the admin gate closure
- `src/Routes.php`: guarded Slim routes
...
route ID on the request state, and the composed psr15-guard middleware builds
its engine request internally, so there is no route-ID hook to reach it. The
engine-sanctioned equivalent used here is the `custom_request` pipeline check:
a `SecurityConfig.customRequestCheck` closure that inspects the `GuardRequest`
and returns a block verdict for `/admin/*` requests missing the right
`X-Admin-Token` header. For Slim-native route scoping, `addToGroup()` can
attach a dedicated guard (its own engine instance) to a route group instead;
group middleware only runs for requests that match a route inside the group.
...
| `REDIS_PREFIX` | `guard_core_slim:` | Key namespace for bans and rate limits |
...
4. `GET /` with `X-Forwarded-For: 203.0.113.7` (trusted compose network resolves it) is blocked `403` with body `Blocked by slim-guard`
