...
final class Config
{
    ...
    /**
     * The composed psr15-guard middleware builds its engine request
     * internally, so the engine's RouteConfig.requiredHeaders route guard is
     * not reachable through the middleware stack. The engine-sanctioned
     * equivalent is the custom_request pipeline check: a closure over the
     * GuardRequest that returns a block verdict for requests that fail the
     * admin gate.
     */
